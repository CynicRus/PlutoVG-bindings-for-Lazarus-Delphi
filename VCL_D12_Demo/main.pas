...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.ExtCtrls, plutovg_api;
...
begin
  ...
  try
    ...
    begin
      ...
      plutovg_canvas_set_color(Canvas, PLUTOVG_BLACK_COLOR);
      str := 'PlutoVG Demo';
      plutovg_canvas_fill_text(Canvas, @str[1], -1, PLUTOVG_TEXT_ENCODING_UTF16, 50, 500);

      // Второй текст с другим цветом и размером
      plutovg_canvas_set_font_size(Canvas, 18);
      plutovg_canvas_set_color(Canvas, PLUTOVG_MAKE_COLOR(0.5, 0, 0.5, 1)); // Фиолетовый
      str1 := 'Графические примитивы';
      plutovg_canvas_fill_text(Canvas, @str1[1], -1, PLUTOVG_TEXT_ENCODING_UTF16, 50, 530);
      plutovg_canvas_restore(Canvas);
    end;
